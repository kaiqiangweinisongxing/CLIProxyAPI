...
type CodexConfig struct {
	...
	// StreamBootstrapTimeout specifies the maximum duration to hold back uncommitted response
	// headers during bootstrap buffering before releasing the stream to the client.
	// Defaults to "10s". Set to "0", "0s", "none", or "unlimited" to disable the time ceiling.
...
const DefaultCodexStreamBootstrapTimeout = 10 * time.Second
...
// Defaults to 10 seconds if unset or invalid. If explicitly set to "0", "0s", "none", "unlimited",
// "disabled", "off", or "never", returns 0 (unlimited time, relying purely on frame and byte caps).
func (c *CodexConfig) StreamBootstrapTimeoutDuration() time.Duration {
	...
	if raw == "0" || strings.EqualFold(raw, "none") || strings.EqualFold(raw, "unlimited") || strings.EqualFold(raw, "disabled") || strings.EqualFold(raw, "off") || strings.EqualFold(raw, "never") {
		return 0
	}
