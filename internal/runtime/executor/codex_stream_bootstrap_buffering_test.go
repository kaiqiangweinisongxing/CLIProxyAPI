...
func TestCodexConfig_StreamBootstrapTimeoutDuration(t *testing.T) {
	tests := []struct {
		...
		{"-5s", 10 * time.Second},
		{"-1", 10 * time.Second},
		{"9223372037", 10 * time.Second},
		{"18446744074", 10 * time.Second},
		{"36028797018963968", 10 * time.Second},
	}
	...
	if got := nilCfg.StreamBootstrapTimeoutDuration(); got != 10*time.Second {
		t.Errorf("nil StreamBootstrapTimeoutDuration = %v, want 10s", got)
	}
}
