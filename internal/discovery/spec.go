...
func BuildTXTRecords(opts TXTOptions) []string {
	...
	for _, item := range candidates {
		...
		if len(entry) > maxTXTRecordBytes {
			entry = entry[:maxTXTRecordBytes]
		}
		entryCost := len(entry) + 1 // +1 for DNS TXT length prefix byte

		if totalBytes+entryCost > maxTXTBytes {
			// Exceeds total budget: drop priority 2 and 3 items
			if item.priority > 1 {
				continue
			}
			// If even priority 1 exceeds, truncate entry if room remains
			allowed := maxTXTBytes - totalBytes - 1
			if allowed > len(item.key)+1 {
				entry = entry[:allowed]
				entryCost = len(entry) + 1
			} else {
				continue
			}
		}
