...
func TestAdvertiserAndBrowser_Integration(t *testing.T) {
	...
	browser := NewZeroconfBrowser()
	results, err := browser.Browse(ctx, DefaultServiceType, DefaultDomain, 2*time.Second)
	if err != nil {
	...
	if !found {
		t.Logf("service not discovered via multicast in test (expected in sandboxed environments)")
	}
}
